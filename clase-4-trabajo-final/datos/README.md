...
- **Participación Ciudadana. Propuestas.** 920 filas completamente vacías, que son también los
  919 duplicados: es un problema, no dos. Fechas como texto, mezclando `dd/mm/aa` y `dd/mm/aaaa`.
  Nombres de columna con espacios, mayúsculas y acentos.
...
- **BAFICI. Films 2015.** Encoding roto en origen y sin arreglo limpio: la columna `aã±o` se
  llama así en el archivo. Un puñado de registros de prueba y basura (`test en prd`, `test`,
  `asdf`, `jhgjg`) mezclados con los films. Duración con la unidad pegada al número (`"76 min"`),
  y en 172 filas con el final del número corrupto (`6m min`, `1h min`, `5: min`).
